...
SELECT PROFESSOR_NAME AS '교수이름',
       TRUNCATE(DATEDIFF(DATE(NOW()),
                         DATE(CONCAT('19', SUBSTRING(CAST(PROFESSOR_SSN AS CHAR(14)), 1, 6))))/365,0) AS '나이'
FROM tb_professor
WHERE SUBSTRING(CAST(PROFESSOR_SSN AS CHAR(14)), 8, 1) = '1'
...
SELECT STUDENT_NAME,
       TRUNCATE(DATEDIFF(DATE(ENTRANCE_DATE),
                         DATE(SUBSTRING(CAST(STUDENT_SSN AS CHAR(14)), 1, 6)))/365, 0) AS '입학나이'
FROM tb_student
WHERE TRUNCATE(DATEDIFF(DATE(ENTRANCE_DATE),
                        DATE(SUBSTRING(CAST(STUDENT_SSN AS CHAR(14)), 1, 6)))/365, 0) > 19;
...
ORDER BY SUBSTRING(TERM_NO,1,4);
...
-- 13. 춘 대학교에 다니는 동명이인 학생들의 이름을 찾고자 한다. 어떤 SQL 문장을 사용하면 가능하겠는가?
